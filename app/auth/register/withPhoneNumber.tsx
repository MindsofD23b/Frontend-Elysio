  ...
  const onSubmit = (data: FormData) => {
    ...
    if (!validation.success) {
      ...
      setErrors((prev) => ({
        ...prev,
        tel: {
          message: validation.error.format().tel?._errors[0] || "Invalid phone number",
        },
      }));
      return;
    }

    const telStriped = data.tel.startsWith("0")
      ? data.tel.startsWith("00")
        ? data.tel.substring(2, data.tel.length)
        : data.tel.substring(1, data.tel.length)
      : data.tel;
      ...
      <BackWrapper>
        <View style={{ flex: 1, flexDirection: "column", width: "100%", height: "100%" }}>
          ...
          <Text style={[gs.bodyText, { marginTop: 10, color: theme.base + "54" }]}>
            Please enter your <Text style={{ fontWeight: "bold" }}>Phone Number</Text>
          </Text>
          <View
            style={{
              width: "100%",
